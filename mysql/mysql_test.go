...
var CD = &ps.ConnData{"host", "3306", "user", "pass", "dbname", ""}
...
func TestInit(t *testing.T) {
	Conn.Connect(CD)
	Conn.Query(&ps.Stmt{make([]string, 0), "CREATE TABLE test(Someid integer primary key auto_increment, SomeText Char(255), i integer);"})
	for i := 0; i < 10; i++ {
...
func TestCustomConnection(t *testing.T) {
	...
	ok := con.Connect(CD)
	if !ok {
		t.Error("Mysql connection cannot be established")
	}
}
